    ...
    <Container className="py-10 overflow-hidden rounded-xl bg-base-100">
      ...
      >
        ...
        {steps.map((step, index) => (
          ...
          >
            {/* Card Container */}
            <div className="relative h-full bg-base-100 border border-base-200 rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden hover:-translate-y-1">
              {/* Subtle Background Tint on Hover */}
              <div
                className={`absolute inset-0 bg-linear-to-br ${step.color} opacity-0 group-hover:opacity-[0.02] transition-opacity duration-300`}
              />

              {/* Step Number Badge - Subtle */}
              <div className="absolute top-4 right-4 text-3xl font-black text-base-content/5 group-hover:text-base-content/10 transition-colors duration-300">
                {step.id}
              </div>
              ...
              >
                <div
                  className={`w-20 h-20 rounded-2xl bg-linear-to-br ${step.color} p-0.5 shadow-md group-hover:shadow-lg transition-all duration-300`}
                >
                  <div className="w-full h-full bg-base-100 rounded-2xl flex items-center justify-center">
                    <step.icon
                      className="text-3xl text-primary group-hover:scale-105 transition-transform duration-300"
                    />
                  </div>
                </div>
              </motion.div>
